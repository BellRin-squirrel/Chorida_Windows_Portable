(function() {
    ...
    Object.assign(m, {
        init: function() {
            console.log("[DEBUG-UI] Initializing Modal UI...");
                    ...
                    if (e.button === 2) {
                        ...
                        if (item) {
                            ...
                            if (found) {
                                ...
                                console.log("[DEBUG-UI] Captured ID from right-click:", found.id);
                            }
                        }
                    }
                });
            ...
            if (menuEditSongs) {
                menuEditSongs.addEventListener('click', () => {
                    console.log("[DEBUG-UI] Menu 'Edit Songs' clicked.");
            ...
            if (checkAllBtn) {
                checkAllBtn.closest('th').addEventListener('click', (e) => {
                    console.log("[DEBUG-UI] Header checkbox clicked.");
        ...
        open: async function(pl_id) {
            console.log("[DEBUG-UI] Modal open for ID:", pl_id);
        ...
        // ★ UI: 並び替えアイコン(矢印)の描画を更新
        updateSortUI: function() {
            console.log("[DEBUG-UI] Updating sort icons...");
            ...
            headers.forEach(th => {
                ...
                // 現在のソート列なら矢印を付与
                // クラス名(col-title等)が現在のsortFieldと一致するかチェック
                if (th.classList.contains(`col-${this.sortField}`)) {
        ...
        renderList: function() {
            ...
            this.filteredData.forEach((item, index) => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="col-title">${u.escapeHtml(item.title || '')}</td>
                    <td class="col-artist">${u.escapeHtml(item.artist || '')}</td>
                    <td class="col-album">${u.escapeHtml(item.album || '')}</td>
                    <td class="col-track">${u.escapeHtml(item.track || '')}</td>
                    <td class="col-genre">${u.escapeHtml(item.genre || '')}</td>
                `;
        ...
        handleRowClick: function(index, event) {
            ...
            if (event.shiftKey && this.lastClickedIndex !== null) {
                console.log(`[DEBUG-UI] Shift selection from ${this.lastClickedIndex} to ${index}`);
