...
def ensure_default_image():
    ...
    if not os.path.exists(target):
        # app/icon/MusicManager.png をコピー
        source = resource_path(os.path.join('app', 'icon', 'MusicManager.png'))
...
def reset_default_artwork():
    """デフォルト画像(default.png)をMusicManager.pngで初期化する"""
    target = os.path.join(IMAGE_DIR, "default.png")
    # app/icon/MusicManager.png の絶対パスを取得
    source = resource_path(os.path.join('app', 'icon', 'MusicManager.png'))
    try:
        if os.path.exists(source):
            shutil.copy(source, target)
...
def get_playlist_details(pl_id):
    ...
    try:
        with open(path, 'r', encoding='utf-8') as f: pl = json.load(f)
        raw_db = load_db()
        ...
        for fname in pl.get('music', []):
            if fname in music_map:
                ...
                # get_image_base64 内で resolve_db_path されるのでここはそのまま渡してOK
...
def get_latest_github_tag(repo):
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0 (MusicManager)'})
    try:
        with urllib.request.urlopen(req) as response:
...
def get_default_export_path():
    ...
    try:
        return os.path.join(os.path.expanduser("~"), "Desktop", "MusicManager_Export.zip")
    except Exception:
        return ""
...
def ask_save_path(current_path):
    ...
    initial_dir = ""
    initial_file = "MusicManager_Export.zip"
    if current_path:
